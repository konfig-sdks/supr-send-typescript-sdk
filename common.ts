...
import { readableStreamToString, SuprSendError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SuprSendError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SuprSendError(e, e.response?.data)
                }
                if (innerError instanceof SuprSendError) {
                    // Got "SuprSendError" from the above try block
                    throw innerError;
                }
